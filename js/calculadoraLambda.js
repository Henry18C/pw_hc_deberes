...
*/
  ...
  //SENTENCIAS DE CONTROL DEBER


  const numerosN = [1, 2, 3, 4, 5];
  const numerosC = ["1", "2", "3", "4", "5"];
  
  for (let i = 0; i < numerosN.length; i++) {
      const numeroN = numerosN[i];
      const numeroC = numerosC[i];
  
      console.log(`Comparando numeroN = ${numeroN} con numeroC = "${numeroC}"`);
  
      // Igualdad no estricta (==)
      if (numeroN == numeroC) {
          console.log(`${numeroN} == "${numeroC}" es true`);
      } else {
          console.log(`${numeroN} == "${numeroC}" es false`);
      }
